...
module's internals; the public surface (`Embedder.embed_document` /
`Embedder.embed_query`) stays the same.
"""
...
class Embedder:
    ...
    async def _call_gemini(self, content: str, *, task_type: str) -> Iterable[float]:
        # We import inside the method so tests can monkeypatch / construct
        # fakes without needing the real SDK at module load time.
        try:
