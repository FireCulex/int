...
| Embedding tasks | `RETRIEVAL_DOCUMENT` for `add`; `RETRIEVAL_QUERY` for `search`/`recall` (`list` needs no embedding) |
...
uv run int-cli recall --project <p> --query "..."
```
...
│   ├── store.py                     # Qdrant client wrapper: add/delete/search/list/recall
...
class Embedder:
    ...
    async def embed_query(self, content: str) -> list[float]:
        # Used by `search`/`recall`. task_type=RETRIEVAL_QUERY.
...
| `int.recall` | `project: str`, `query: str`, `limit: int = 5` | `list[SearchResult]` (same as `search`) | Alias for `search` with higher limit. Reserved for future "summary + search" behavior; v1 is a thin pass-through. |
...
- Every `search`/`recall` result respects the `project` filter — zero cross-project leaks. Integration test stores in A, queries B, asserts empty.
...
- Adding a new MCP tool outside v1's five (`add`/`delete`/`search`/`list`/`recall`).
...
1. **Five-tool MCP surface works.** `add`, `delete`, `search`, `list`, `recall` are all callable from an MCP client against the running server. `search` returns ranked results; `list` returns metadata without content.
...
6. **Offline-degrade is explicit.** If `GEMINI_API_KEY` is missing or the API is unreachable, `add`/`search`/`recall` return a typed `EmbeddingError` (not a crash). `list` (no embedding needed) still works.
