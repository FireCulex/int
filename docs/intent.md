...
An assistant, working in a project, prompted to re-derive something a prior session already discovered, calls `int.search` first and answers from stored memory instead of re-running the discovery. Concretely: given a stored architecture synthesis for a project, a semantic query returns it in the top 3 results.
